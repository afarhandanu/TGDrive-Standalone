...
Konfigurasi Google Cloud harus cocok dengan APK The Great Drive yang ditandatangani oleh kunci rilis lama. Pemilih akun yang tertutup setelah akun diketuk belum membuktikan bahwa pengguna membatalkan: Google juga dapat menolak otorisasi yang belum dikonfigurasi.
...
5. Pastikan APK yang diuji ditandatangani oleh keystore rilis yang sama. SHA-1 di atas adalah sidik jari sertifikat yang tersimpan dalam `TGDrive-signing-key.jks`, bukan SHA-256 berkas keystore. Jangan buat kunci baru atau client OAuth baru hanya karena nama aplikasi berubah.
